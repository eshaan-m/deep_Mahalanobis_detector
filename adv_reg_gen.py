...
def main():
	...
	for dataset in dataset_list:
		...
		# gmm_outf = './gen_output/' + args.net_type + '_' + dataset + '/'
		gmm_outf = './gmm_multiclass_output/' + args.net_type + '_' + dataset + '/'

		########################
		n_components = 1
		if class_conditional_gau_f == False:
		pkl_filename = gmm_outf + 'gmm_'+ str(n_components)+'_'+dataset+'.pkl'
		...
		if class_conditional_gau_f == True:
			gmm = []
			for i in range(10):
				pkl_filename = gmm_outf+'gmm_'+str(n_components)+'_'+dataset+'class_'+str(i)+'.pkl'
