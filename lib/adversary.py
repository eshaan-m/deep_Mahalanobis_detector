...
def deepfool_single(model, imgs, target, n_classes, train_mode, max_iter=50,
    ...
    for m in range(max_iter):
        ...
        min_norm = min_norm.item()      #min_norm = min_norm[0]
        ...
        _, pred2 = output2.data.cpu().max(1)
        pred2 = pred2.squeeze().item()     #pred2 = pred2.squeeze()[0]
        diff = torch.norm(imgs_var - imgs_var2) / torch.norm(imgs_var)
        diff = diff.data.item()     #diff = diff.data[0]
        if verbose:
...
def cw(model, input, target, weight, loss_str, bound=0, tv_weight=0,
    ...
    for i in range(max_iter):
        ...
        recons_loss = loss.data.item()      #recons_loss = loss.data[0]
        w_data = w.data
        ...
        adv_loss = loss.data.item() - recons_loss   #adv_loss = loss.data[0] - recons_loss
        if is_gpu:
            loss = loss.cuda()
        loss.backward()
        if crop_frac < 1 and i % 3 == 1:
        ...
        optimizer.step()
        total_loss = loss.data.cpu().item() + tv_loss       # loss.data.cpu()[0] + tv_loss
