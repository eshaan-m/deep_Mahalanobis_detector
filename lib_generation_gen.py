...
def sample_estimator(model, num_classes, feature_list, train_loader, outf):
	"""
	...
	#####################################
	for i in range(num_output):
		transform_filename = outf + 'dim_transformer_' + str(i) + '.pkl'
		transformer = pickle.load(open(transform_filename, 'rb'))
		transform_list.append(transformer)
	...
	for data, target in train_loader:
		...
		for i in range(num_output):
			...
			transform_matrix = torch.from_numpy(transform_list[i].components_)
			transform_matrix = transform_matrix.type(torch.FloatTensor)
			transform_matrix = transform_matrix.cuda()
			out_features[i] = out_features[i].view(out_features[i].size(0),-1)
			out_features[i] = torch.mm(out_features[i].data, transform_matrix.t())
...
def get_activations(model, test_loader, outf, layer_index):
	'''
	Extract the activations for all the batches for a layer and dump in a .pkl
	return: None
	'''
	model.eval()
	pkl_file_name = outf + 'activtions_in_' + str(layer_index) +'.pkl'
	act_file = open(pkl_file_name, 'ab+')  ######### file to  activations
	for data, target in test_loader:
		...
		pickle.dump(out_features.cpu().detach().numpy(), act_file)
...
def get_Mahalanobis_score(model, test_loader, num_classes, outf, out_flag, net_type, sample_mean, precision, layer_index, magnitude):
	...
	transform_filename = outf + 'dim_transformer_' + str(layer_index) + '.pkl'
	transformer = pickle.load(open( transform_filename, 'rb'))
	transform_matrix = torch.from_numpy(transformer.components_)
	transform_matrix = transform_matrix.type(torch.FloatTensor)
	transform_matrix = transform_matrix.cuda()
	...
	for data, target in test_loader:
		...
		out_features = torch.mm(out_features, transform_matrix.t())
		...
		noise_out_features = noise_out_features.view(noise_out_features.size(0), -1)
		noise_out_features = torch.mm(noise_out_features, transform_matrix.t())
		...
		### edits : comment out the max step
