...
## taken from OOD_Regression_Mahalanobis.py
from __future__ import print_function
import numpy as np
import os
import lib_reg_gen as lib_regression
import argparse
...
def main():
	...
	for dataset in dataset_list:
		...
		# outf = './gen_output/' + args.net_type + '_' + dataset + '/'
		outf = './gmm_multiclass_output/' + args.net_type + '_' + dataset + '/'
		...
		if class_conditional_gau_f == False:
			pkl_filename = outf+'gmm_'+str(n_components)+'_'+dataset+'.pkl'
			# pkl_filename = outf+'gmm_'+dataset+'.pkl'

			gmm = pickle.load(open(pkl_filename, 'rb'))
			
		if class_conditional_gau_f == True:
			gmm = []
			for i in range(100):
				pkl_filename = outf+'gmm_'+str(n_components)+'_'+dataset+'class_'+str(i)+'.pkl'
				gmm.append(pickle.load(open(pkl_filename, 'rb' )))
		...
		for out in out_list:
			...
			for score in score_list:
				...
				results = lib_regression.detection_performance(gmm, X_val_for_test, Y_val_for_test, outf)
				print(results)
				...
				if best_tnr < results['TMP']['TNR']:
					...
					best_result = lib_regression.detection_performance(gmm, X_test, Y_test, outf)
